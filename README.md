...
- There are buttons for increment/decrement the score and to reset the highlighting of the fastest one that pushes the big red button.
- NodeJs is used for reading the wired-usb playstation buzzers and communication is done via websockets for updating the webapplication

Many thanks to the creators from [project](https://www.hakantuncer.com/2016/09/07/creating-an-online-quiz-game-using-node-dot-js) for reading and parsing the bytestream that the buzzers generate. That project is the starting point for this project.
